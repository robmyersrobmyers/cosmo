...
A filesystem storage provider works the same way, with no S3 or CDN access required. This is the only option when a subgraph is itself a router that needs to enforce [`block_non_persisted_operations`](#disallowing-non-persisted-operations) against the subquery documents a gateway sends it, rather than against client operations. See [Exporting Subgraph Persisted Operations](/router/persisted-queries/exporting-subgraph-persisted-operations) for how to generate those files.
