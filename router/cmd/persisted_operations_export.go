...
import (
	...
	"github.com/wundergraph/cosmo/router/pkg/persisted_operations_export"
)

func PersistedOperationsExport(args []string) {
	var exportHelp bool

	cfg := persisted_operations_export.ExportConfig{}
	f := flag.NewFlagSet("router "+args[0], flag.ExitOnError)
	f.BoolVar(&exportHelp, "help", false, "Prints the help message")
	...
	f.StringVar(&cfg.OutDir, "out-dir", "", "required, output folder location; one subdirectory per subgraph name is created")
	...
	f.IntVar(&cfg.Concurrency, "concurrency", 0, "how many operations to plan concurrently")
	f.BoolVar(&cfg.OutputReport, "print-report", false, "write a report.json file, with the subgraphs touched by each operation, sorted by file name")
	f.BoolVar(&cfg.FailOnPlanError, "fail-on-error", false, "if at least one operation fails to export, the command exit code will be 1")
	f.BoolVar(&cfg.FailFast, "fail-fast", false, "stop as soon as possible if an operation fails to export")
	...
	logger := logging.New(false, false, true, logLevel).
		With(
			zap.String("service", "@wundergraph/export-persisted-operations"),
	...
	err = persisted_operations_export.Export(ctxNotify, cfg)
	if err != nil {
		logger.Fatal("Error during command export-persisted-operations: %s", zap.Error(err))
	}
}
